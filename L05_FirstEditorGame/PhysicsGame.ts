namespace L05_PhysicsGame {
  ...
  let audioBackground: ƒ.Audio = new ƒ.Audio("./music/backgroundmusic.mp3");
  let audioGrab: ƒ.Audio = new ƒ.Audio("./music/grab.wav");
  let audioShoot: ƒ.Audio = new ƒ.Audio("./music/shoot.wav");
  let rootGraph: ƒ.Graph;
  let cmpAvatar: ƒ.ComponentRigidbody;
  let cmpRigidbodyBall: ƒ.ComponentRigidbody;
  let ball: ƒ.Node;
  let avatarNode: ƒ.Node;
  let childAvatarNode: ƒ.Node;
  ...
  let distance: ƒ.Vector3;
  let kickStrength: number = 750;
  let isGrabbed: boolean;
  let mouseMove: ƒ.Vector2 = new ƒ.Vector2();
  let isMouseMooving: boolean;
  let cmpAudioGrab: ƒ.ComponentAudio = new ƒ.ComponentAudio(audioGrab);
  let cmpAudioShoot: ƒ.ComponentAudio = new ƒ.ComponentAudio(audioShoot);
  ...
  async function start(_event: Event): Promise<void> {
    ...
    rootGraph = <ƒ.Graph>FudgeCore.Project.resources["Graph|2021-04-27T14:37:42.239Z|64317"];
  ...
  function setupAudio(): void {
    ...
    let audioNode: ƒ.Node = new ƒ.Node("audioNode");
    let cmpAudioBackground: ƒ.ComponentAudio = new ƒ.ComponentAudio(audioBackground, true, true);
  ...
  function createRigidbodies(): void {
    let level: ƒ.Node = rootGraph.getChildrenByName("level")[0];
    ...
    if (cmpRigidbodyBall != undefined) {
      ...
      if (ƒ.Keyboard.isPressedOne([ƒ.KEYBOARD_CODE.R]) && isGrabbed == true) {
        ...
        cmpRigidbodyBall.applyImpulseAtPoint(
          new ƒ.Vector3(playerForward.x * kickStrength, playerForward.y * 5 * kickStrength, playerForward.z * kickStrength),
